...
var telefonos = async () => {
    ...
    myHeader.set("accept", "ListarTelefonos");
    let config = {
        headers: myHeader,
        method: URI.method
    };
...
document.querySelector("tbody").addEventListener("click", async (e) => {
    if (!e.target.classList.value.indexOf("edit")) {
        ...
        let json = {
            ...
            telefono: input.children[1].innerHTML,
        ...
        myHeader.set("accept", "UpdateTelefono");
        ...
        let myHeader = new Headers();
        myHeader.set("accept", "TelefonoDelete");
        let config = {
            headers: myHeader,
            method: "POST",
            body: JSON.stringify(json)
        ...
        if (res["@Mensaje"]) {
            Swal.fire({
                position: 'center',
                icon: 'error',
                title: `El telefono ${input.children[1].innerHTML} ha sido eliminado`,
                showConfirmButton: false,
                timer: 3000
            })
            input.remove();
