...
var MatDevuelto = async () => {
    let myHeader = new Headers();
    myHeader.set("accept", "ListarMaterialDevuelto");
    let config = {
        headers: myHeader,
        method: URI.method
    };
...
document.querySelector("tbody").addEventListener("click", async (e) => {
    if (!e.target.classList.value.indexOf("edit")) {
        ...
        myHeader.set("accept", "updateMatDevuelt");
        ...
        let myHeader = new Headers();
        myHeader.set("accept", "deleteMatDevuelt");
        let config = {
            headers: myHeader,
            method: "POST",
            body: JSON.stringify(json)
        ...
        if (res["Datos eliminados"]) {
            Swal.fire({
                position: 'center',
                icon: 'error',
                title: `El Material Devuelto ha sido eliminado`,
                showConfirmButton: false,
                timer: 3000
            })
            input.remove();
