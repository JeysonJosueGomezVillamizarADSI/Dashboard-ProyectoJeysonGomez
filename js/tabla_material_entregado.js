...
var MatEntregado = async () => {
    let myHeader = new Headers();
    myHeader.set("accept", "ListarMaterialEntregado");
    let config = {
        headers: myHeader,
        method: URI.method
    };
...
document.querySelector("tbody").addEventListener("click", async (e) => {
    if (!e.target.classList.value.indexOf("edit")) {
        ...
        let json = {
            ...
            Cantidad: input.children[1].innerHTML,
            Fecha: input.children[2].innerHTML,
            Hora: input.children[3].innerHTML,
        }
        let myHeader = new Headers();
        let config = {
            headers: myHeader,
        ...
        myHeader.set("accept", "updateMatEntreg");
        ...
        let myHeader = new Headers();
        myHeader.set("accept", "deleteMatEntreg");
        let config = {
            headers: myHeader,
            method: "POST",
            body: JSON.stringify(json)
        ...
        if (res["Datos eliminados"]) {
            Swal.fire({
                position: 'center',
                icon: 'error',
                title: `El Material Entregado  ha sido eliminado`,
                showConfirmButton: false,
                timer: 3000
            })
            input.remove();
