...
var Material = async () => {
    ...
    myHeader.set("accept", "materialgetALL");
...
document.querySelector("tbody").addEventListener("click", async (e) => {
    if (!e.target.classList.value.indexOf("edit")) {
        ...
        let json = {
            ...
            material: input.children[1].innerHTML,
            descrip: input.children[2].innerHTML,
            precio: input.children[3].innerHTML,
            cantidad: input.children[4].innerHTML,
        }
        let myHeader = new Headers();
        let config = {
            headers: myHeader,
        ...
        myHeader.set("accept", "UpdateMaterial");
        let peticion = await fetch("../php/api.php", config);
        let res = await peticion.json();
