    ...
    return new Promise((resolve) => {
        ...
        for (let [id, fila] of Object.entries(data)) {
            ...
            for (let [name, columna] of Object.entries(fila)) {
                ...
                if (name.replace(/\s+/g, '').substring(name.length - 3).toUpperCase() == "FK") {
...
var Roles = async () => {
    ...
    myHeader.set("accept", "ListarRoles");
    let config = {
        headers: myHeader,
        method: URI.method
    };
...
document.querySelector("tbody").addEventListener("click", async (e) => {
    if (!e.target.classList.value.indexOf("edit")) {
        ...
        let json = {
            id_rol: input.children[0].innerHTML,
            rol: input.children[1].innerHTML,
            // nom: input.children[2].innerHTML,
        }
        let myHeader = new Headers();
        let config = {
            headers: myHeader,
        ...
        if (res["Datos eliminados"]) {
            Swal.fire({
                position: 'center',
                icon: 'error',
                title: `El rol ${input.children[1].innerHTML} ha sido eliminado`,
                showConfirmButton: false,
                timer: 3000
            })
            input.remove();
