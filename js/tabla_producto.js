...
var Productos= async () => {
    ...
    myHeader.set("accept", "ListarProductos");
...
document.querySelector("tbody").addEventListener("click", async (e) => {
    if (!e.target.classList.value.indexOf("edit")) {
        ...
        let json = {
            ...
            produc: input.children[1].innerHTML,
            pares: input.children[2].innerHTML,
            descrip: input.children[3].innerHTML,
            talla: input.children[4].innerHTML,
            color: input.children[5].innerHTML,
        }
        let myHeader = new Headers();
        let config = {
            headers: myHeader,
        ...
        myHeader.set("accept", "UpdateProductos");
        let peticion = await fetch("../php/api.php", config);
        let res = await peticion.json();
