...
var TareaProducto = async () => {
    let myHeader = new Headers();
    myHeader.set("accept", "ListarTareaProducto");
    let config = {
        headers: myHeader,
        method: URI.method
    };
...
document.querySelector("tbody").addEventListener("click", async (e) => {
    if (!e.target.classList.value.indexOf("edit")) {
        ...
        let json = {
            ...
            inicio: input.children[4].innerHTML,
            fin: input.children[5].innerHTML,
            cant: input.children[6].innerHTML,
        }
        let myHeader = new Headers();
        let config = {
            headers: myHeader,
        ...
        myHeader.set("accept", "updateTareaProd");
